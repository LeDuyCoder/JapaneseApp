...
import 'package:japaneseapp/features/achivement/domain/usecase/download_topic_usecase.dart';
...
class DowloadTopicBloc extends Bloc<DowloadTopicEvent, DowloadTopicState>{
  ...
  Future<void> _onLoad(DowloadTopicLoad event, Emitter<DowloadTopicState> emit) async {
    try{
      await repository.loadData(event.topicId).then((feachData) async {
        EffectReward? effectReward;

        await DownloadTopicUsecase().call().then((reward){
          if(reward != null){
            effectReward = reward;
          }
        });

        emit(DowloadTopicLoadState(effectReward, dowloadTopicEntity: feachData));
      }).catchError((e){
